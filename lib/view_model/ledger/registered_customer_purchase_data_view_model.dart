...
class RegisteredCustomerPurchaseDataViewModel {
  ...
  Future<bool> closeAPurchaseRecordInLedger(
      Map<String, dynamic> closePurchaseData, BuildContext context) async {
    try {
      bool status = await _ledgerRepository
          .closeAPurchaseRecordInLedger(closePurchaseData);

      return true;
    } catch (error) {
      Future.delayed(Duration.zero, () {
        AppUtils.flushBarSucessMessage(error.toString(), context);
      });
      rethrow;
    }
  }
